...
var lex = lq.parse(lexer, "name", source);
if (lex.succeeded) {
  var parse = lq.parse(stylesheet, "name", lex.value);
  if (parse.succeeded) {
    console.log(parse.value.toString());
  }
  else {
  ...
  console.log(lex.error.toString());
}