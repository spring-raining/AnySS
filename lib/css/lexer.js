...
var unicodeRangeParser = lq.oneOf("Uu")
  .bind(function(u) {
    return lq.char("+")
      .bind(function(n) {
        return lq.choice([
          countFromTo(1, 6, hexDigit).bind(function(a) {
            return lq.char("-").bind(function(b) {
              return countFromTo(1, 6, hexDigit).bind(function(c) {
                return lq.pure(a.join("") + b + c.join(""));
              });
            });
          }).try(),
          countFromTo(0, 5, hexDigit).bind(function(a) {
            return countFromTo(1, (6 - d.length), lq.char("?")).bind(function(b) {
              return lq.pure(a.join("") + b);
            });
          }).try(),
          countFromTo(1, 6, hexDigit).bind(function(a) {
            return lq.pure(q.join(""))
          })
        ]).bind(function(i) {
          return lq.pure(u + n + i);
        })
      })
  })
  .try();
var unicodeRangeToken = lq.getPosition
  .bind(function(pos) {
    return unicodeRangeParser.bind(function(v) {
      return lq.pure(new tokens.UnicodeRangeToken(pos, v));
    });
  });
...
var closeBraceToken   = symbol(lq.char("}"), tokens.CloseBraceToken);
